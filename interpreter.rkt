...
(define (bool? expr) 
    (and    (list? expr) 
            (contains? (operator expr) '(true false == < > <= >= || &&))))
(define (arith? expr) 
    (or (number? expr) 
        (and    (list? expr) 
...
(define (M_int expr state)
  (cond
    ...
    [(and   (eq? '+ (operator expr))
            (not (signed? expr)))   (+  (M_int (leftoperand expr) state)
                                        (M_int (rightoperand expr) state))]
    [(and   (eq? '+ (operator expr))
            (signed? expr))         (M_int (leftoperand expr) state)]
    [(and   (eq? '- (operator expr))
            (not (signed? expr)))   (-  (M_int (leftoperand expr) state)
                                        (M_int (rightoperand expr) state))]
    [(and   (eq? '- (operator expr))
            (signed? expr))         (-  0 
                                        (M_int (leftoperand expr) state))]
    [(eq? '* (operator expr))       (*  (M_int (leftoperand expr) state)
                                        (M_int (rightoperand expr) state))]
    [(eq? '/ (operator expr))       (quotient   (M_int (leftoperand expr) state)
                                                (M_int (rightoperand expr) state))]
    [(eq? '% (operator expr))       (remainder  (M_int (leftoperand expr) state)
                                                (M_int (rightoperand expr) state))]))
; M_boolean
(define M_boolean
  (lambda (expr state)
    (cond
      ...
      [(eq? 'false expr)            #f]
      [(not (list? expr))           (getval expr state)]
      [(eq? '! (operator expr))     (not (M_boolean (leftoperand expr) state))]
      [(eq? '== (operator expr))    (eq?    (M_int (leftoperand expr) state)
                                            (M_int (rightoperand expr) state))]
      [(eq? '< (operator expr))     (<      (M_int (leftoperand expr) state)
                                            (M_int (rightoperand expr) state))]
      [(eq? '> (operator expr))     (>      (M_int (leftoperand expr) state)
      ...
      [(eq? '>= (operator expr))    (>=     (M_int (leftoperand expr) state)
                                            (M_int (rightoperand expr) state))]
      [(eq? '|| (operator expr))    (or     (M_boolean (leftoperand expr) state)
                                            (M_boolean (rightoperand expr) state))]
      [(eq? '&& (operator expr))    (and    (M_boolean (leftoperand expr) state)
                                            (M_boolean (rightoperand expr) state))]
      )))
...
(define (M_state_if stmt state)
  (cond
    [(M_value (condition stmt) state)   (M_state_stmt (stmt1 stmt) state)]
    [(eq? (len stmt 0) 4)               (M_state_stmt (stmt2 stmt) state)]
    [else                               state]))
...
(define (M_state_while stmt state)
  (cond
    [(M_boolean (loop_condition stmt) state)    (M_state_while  stmt 
